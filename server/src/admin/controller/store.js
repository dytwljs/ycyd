...
module.exports = class extends Base {
  ...
  async indexAction() {
    ...
    const data =await model.field(['*']).where({ 'title':['like', `%${name}%`]}).page(page,size).countSelect();
  ...
  async indexDetailAction() {
    ...
    const data =await model.field(['*']).where({ 'store_enter_id':store_enter_id}).select();
    // const data = await model.field(['nideshop_goods.name as goods_name','nideshop_store_enter_detail.*']).join('nideshop_goods ON nideshop_store_enter_detail.goods_id=nideshop_goods.id').order(['nideshop_store_enter_detail.id DESC'])select();
  ...
  async infoAction() {
    ...
    const store_house = await this.model('store_house')
      .field(['*'])
      .select();
      data.allCategory={brand,store_house};
    return this.success({data});
  }
  ...
  async AttributesAction() {
    ...
    const store_house = await this.model('store_house')
      .field(['*'])
      .select();
    return this.success({ brand, store_house });
  }
  ...
  async destoryAction() {
    const id = this.post('id');
    //  标记删除，商品一般情况下不能真正删除。
    await this.model('store_enter').where({id: id}).limit(1).delete();
    return this.success();
  }
  ...
  async destoryDetailAction() {
    const id = this.post('id');
    //  标记删除，商品一般情况下不能真正删除。
    await this.model('store_enter_detail').where({id: id}).delete();
    return this.success();
  }
};
